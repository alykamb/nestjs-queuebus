...
export class BullMq implements OnModuleDestroy {
    ...
    constructor(@Inject(QUEUE_CONFIG_SERVICE) private readonly queueConfig: IQueueConfigService) {
        this.redis = {
            host: this.queueConfig.redisConfig?.host || 'localhost',
            port: this.queueConfig.redisConfig?.port || 6379,
        }
     ...
     * @param jobOpts - opções de Jobs do bulljs
     */
    public addJob<TRet = any, TData = any>(
        ...
        return from(this.queues.get(module).add(name, data, jobOpts)).pipe(
    ...
    public createQueue(module: string): void {
        ...
        //cria streams para a queue
        this.completedJobs.set(module, new Subject())
        this.failedJobs.set(module, new Subject())
        ...
        this.queueEvents.get(module).on('failed', (job) => {
            this.failedJobs.get(module).next({ ...job, event: JobEventType.failed })
        })
    }
